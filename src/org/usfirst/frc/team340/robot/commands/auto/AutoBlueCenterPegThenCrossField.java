...
public class AutoBlueCenterPegThenCrossField extends CommandGroup {
    ...
    public AutoBlueCenterPegThenCrossField() {
    	...
    	//drive forward
    	addSequential(new DriveStraightToDistance(69,.6*RobotMap.SPEED_SCALE),4);
    	//wait a moment
    	...
    	addSequential(new DriveStraightToDistance(130,.6*RobotMap.SPEED_SCALE),6);
    	//turn to drive across field
    	addSequential(new DriveTurnToAngle(-90),4);
    	//drive across field
    	//TODO:increase the distance here
    	addSequential(new DriveStraightToDistance(-10,-.7*RobotMap.SPEED_SCALE),6);
    }
}
